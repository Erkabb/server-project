...
const SignUp = () => {
    ...
    const formSchema = z.object({
        firstname: z.string().min(3).nonempty({message: 'Must have a value'}),
        lastname: z.string().min(3).nonempty({message: 'Must have a value'}),
        ...
        <div className="w-full px-10 lg:h-[750px]">
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className='w-full rounded-xl flex h-full'>
                    ...
                    <div className='w-1/2 bg-[#FFFFFE] rounded-e-xl flex flex-col justify-center gap-5 px-10'>
                        {input.map((input) => (
